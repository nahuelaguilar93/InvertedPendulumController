/***********************************************************************************************************************
 * This file was generated by the MCUXpresso Config Tools. Any manual edits made to this file
 * will be overwritten if the respective MCUXpresso Config Tools is used to update this file.
 **********************************************************************************************************************/

#ifndef _PIN_MUX_H_
#define _PIN_MUX_H_

/***********************************************************************************************************************
 * Definitions
 **********************************************************************************************************************/

/*! @brief Direction type  */
typedef enum _pin_mux_direction
{
    kPIN_MUX_DirectionInput = 0U,        /* Input direction */
    kPIN_MUX_DirectionOutput = 1U,       /* Output direction */
    kPIN_MUX_DirectionInputOrOutput = 2U /* Input or output direction */
} pin_mux_direction_t;

/*!
 * @addtogroup pin_mux
 * @{
 */

/***********************************************************************************************************************
 * API
 **********************************************************************************************************************/

#if defined(__cplusplus)
extern "C" {
#endif

/*!
 * @brief Calls initialization functions.
 *
 */
void BOARD_InitBootPins(void);

#define SOPT5_UART0TXSRC_UART_TX 0x00u /*!<@brief UART 0 transmit data source select: UART0_TX pin */

/*! @name PORTC1 (number 71), J1[5]
  @{ */
#define BOARD_My_PWM_PERIPHERAL FTM0   /*!<@brief Device name: FTM0 */
#define BOARD_My_PWM_SIGNAL CH         /*!<@brief FTM0 signal: CH */
#define BOARD_My_PWM_PORT PORTC        /*!<@brief PORT device name: PORTC */
#define BOARD_My_PWM_PIN 1U            /*!<@brief PORTC pin index: 1 */
#define BOARD_My_PWM_CHANNEL 0         /*!<@brief FTM0 channel: 0 */
#define BOARD_My_PWM_PIN_NAME FTM0_CH0 /*!<@brief Pin name */
#define BOARD_My_PWM_LABEL "J1[5]"     /*!<@brief Label */
#define BOARD_My_PWM_NAME "My_PWM"     /*!<@brief Identifier name */
                                       /* @} */

/*! @name PORTD2 (number 95), J2[8]
  @{ */
#define BOARD_UART2_RX_PERIPHERAL SPI0    /*!<@brief Device name: SPI0 */
#define BOARD_UART2_RX_SIGNAL SOUT        /*!<@brief SPI0 signal: SOUT */
#define BOARD_UART2_RX_PORT PORTD         /*!<@brief PORT device name: PORTD */
#define BOARD_UART2_RX_PIN 2U             /*!<@brief PORTD pin index: 2 */
#define BOARD_UART2_RX_PIN_NAME SPI0_SOUT /*!<@brief Pin name */
#define BOARD_UART2_RX_LABEL "J2[8]"      /*!<@brief Label */
#define BOARD_UART2_RX_NAME "UART2_RX"    /*!<@brief Identifier name */
                                          /* @} */

/*! @name PORTD3 (number 96), J2[10]
  @{ */
#define BOARD_UART2_TX_PERIPHERAL SPI0   /*!<@brief Device name: SPI0 */
#define BOARD_UART2_TX_SIGNAL SIN        /*!<@brief SPI0 signal: SIN */
#define BOARD_UART2_TX_PORT PORTD        /*!<@brief PORT device name: PORTD */
#define BOARD_UART2_TX_PIN 3U            /*!<@brief PORTD pin index: 3 */
#define BOARD_UART2_TX_PIN_NAME SPI0_SIN /*!<@brief Pin name */
#define BOARD_UART2_TX_LABEL "J2[10]"    /*!<@brief Label */
#define BOARD_UART2_TX_NAME "UART2_TX"   /*!<@brief Identifier name */
                                         /* @} */

/*! @name PORTC9 (number 81), J1[9]
  @{ */
#define BOARD_My_DIRECTION_PERIPHERAL GPIOC                   /*!<@brief Device name: GPIOC */
#define BOARD_My_DIRECTION_SIGNAL GPIO                        /*!<@brief GPIOC signal: GPIO */
#define BOARD_My_DIRECTION_GPIO GPIOC                         /*!<@brief GPIO device name: GPIOC */
#define BOARD_My_DIRECTION_GPIO_PIN 9U                        /*!<@brief PORTC pin index: 9 */
#define BOARD_My_DIRECTION_PORT PORTC                         /*!<@brief PORT device name: PORTC */
#define BOARD_My_DIRECTION_PIN 9U                             /*!<@brief PORTC pin index: 9 */
#define BOARD_My_DIRECTION_CHANNEL 9                          /*!<@brief GPIOC GPIO channel: 9 */
#define BOARD_My_DIRECTION_PIN_NAME PTC9                      /*!<@brief Pin name */
#define BOARD_My_DIRECTION_LABEL "J1[9]"                      /*!<@brief Label */
#define BOARD_My_DIRECTION_NAME "My_DIRECTION"                /*!<@brief Identifier name */
#define BOARD_My_DIRECTION_DIRECTION kPIN_MUX_DirectionOutput /*!<@brief Direction */
                                                              /* @} */

/*!
 * @brief Configures pin routing and optionally pin electrical features.
 *
 */
void BOARD_InitPins(void);

/*! @name PORTC6 (number 78), U8[11]/SW2
  @{ */
#define BOARD_SW2_PERIPHERAL GPIOC                  /*!<@brief Device name: GPIOC */
#define BOARD_SW2_SIGNAL GPIO                       /*!<@brief GPIOC signal: GPIO */
#define BOARD_SW2_GPIO GPIOC                        /*!<@brief GPIO device name: GPIOC */
#define BOARD_SW2_GPIO_PIN 6U                       /*!<@brief PORTC pin index: 6 */
#define BOARD_SW2_PORT PORTC                        /*!<@brief PORT device name: PORTC */
#define BOARD_SW2_PIN 6U                            /*!<@brief PORTC pin index: 6 */
#define BOARD_SW2_CHANNEL 6                         /*!<@brief GPIOC GPIO channel: 6 */
#define BOARD_SW2_PIN_NAME PTC6                     /*!<@brief Pin name */
#define BOARD_SW2_LABEL "U8[11]/SW2"                /*!<@brief Label */
#define BOARD_SW2_NAME "SW2"                        /*!<@brief Identifier name */
#define BOARD_SW2_DIRECTION kPIN_MUX_DirectionInput /*!<@brief Direction */
                                                    /* @} */

/*! @name PORTA4 (number 38), SW3
  @{ */
#define BOARD_SW3_PERIPHERAL GPIOA                  /*!<@brief Device name: GPIOA */
#define BOARD_SW3_SIGNAL GPIO                       /*!<@brief GPIOA signal: GPIO */
#define BOARD_SW3_GPIO GPIOA                        /*!<@brief GPIO device name: GPIOA */
#define BOARD_SW3_GPIO_PIN 4U                       /*!<@brief PORTA pin index: 4 */
#define BOARD_SW3_PORT PORTA                        /*!<@brief PORT device name: PORTA */
#define BOARD_SW3_PIN 4U                            /*!<@brief PORTA pin index: 4 */
#define BOARD_SW3_CHANNEL 4                         /*!<@brief GPIOA GPIO channel: 4 */
#define BOARD_SW3_PIN_NAME PTA4                     /*!<@brief Pin name */
#define BOARD_SW3_LABEL "SW3"                       /*!<@brief Label */
#define BOARD_SW3_NAME "SW3"                        /*!<@brief Identifier name */
#define BOARD_SW3_DIRECTION kPIN_MUX_DirectionInput /*!<@brief Direction */
                                                    /* @} */

/*!
 * @brief Configures pin routing and optionally pin electrical features.
 *
 */
void BOARD_InitButtons(void);

/*! @name PORTB21 (number 67), D12[3]/LEDRGB_BLUE
  @{ */
#define BOARD_LED_BLUE_PERIPHERAL GPIOB                   /*!<@brief Device name: GPIOB */
#define BOARD_LED_BLUE_SIGNAL GPIO                        /*!<@brief GPIOB signal: GPIO */
#define BOARD_LED_BLUE_GPIO GPIOB                         /*!<@brief GPIO device name: GPIOB */
#define BOARD_LED_BLUE_GPIO_PIN 21U                       /*!<@brief PORTB pin index: 21 */
#define BOARD_LED_BLUE_PORT PORTB                         /*!<@brief PORT device name: PORTB */
#define BOARD_LED_BLUE_PIN 21U                            /*!<@brief PORTB pin index: 21 */
#define BOARD_LED_BLUE_CHANNEL 21                         /*!<@brief GPIOB GPIO channel: 21 */
#define BOARD_LED_BLUE_PIN_NAME PTB21                     /*!<@brief Pin name */
#define BOARD_LED_BLUE_LABEL "D12[3]/LEDRGB_BLUE"         /*!<@brief Label */
#define BOARD_LED_BLUE_NAME "LED_BLUE"                    /*!<@brief Identifier name */
#define BOARD_LED_BLUE_DIRECTION kPIN_MUX_DirectionOutput /*!<@brief Direction */
                                                          /* @} */

/*! @name PORTB22 (number 68), D12[1]/LEDRGB_RED
  @{ */
#define BOARD_LED_RED_PERIPHERAL GPIOB                   /*!<@brief Device name: GPIOB */
#define BOARD_LED_RED_SIGNAL GPIO                        /*!<@brief GPIOB signal: GPIO */
#define BOARD_LED_RED_GPIO GPIOB                         /*!<@brief GPIO device name: GPIOB */
#define BOARD_LED_RED_GPIO_PIN 22U                       /*!<@brief PORTB pin index: 22 */
#define BOARD_LED_RED_PORT PORTB                         /*!<@brief PORT device name: PORTB */
#define BOARD_LED_RED_PIN 22U                            /*!<@brief PORTB pin index: 22 */
#define BOARD_LED_RED_CHANNEL 22                         /*!<@brief GPIOB GPIO channel: 22 */
#define BOARD_LED_RED_PIN_NAME PTB22                     /*!<@brief Pin name */
#define BOARD_LED_RED_LABEL "D12[1]/LEDRGB_RED"          /*!<@brief Label */
#define BOARD_LED_RED_NAME "LED_RED"                     /*!<@brief Identifier name */
#define BOARD_LED_RED_DIRECTION kPIN_MUX_DirectionOutput /*!<@brief Direction */
                                                         /* @} */

/*! @name PORTE26 (number 33), J2[1]/D12[4]/LEDRGB_GREEN
  @{ */
#define BOARD_LED_GREEN_PERIPHERAL GPIOE                   /*!<@brief Device name: GPIOE */
#define BOARD_LED_GREEN_SIGNAL GPIO                        /*!<@brief GPIOE signal: GPIO */
#define BOARD_LED_GREEN_GPIO GPIOE                         /*!<@brief GPIO device name: GPIOE */
#define BOARD_LED_GREEN_GPIO_PIN 26U                       /*!<@brief PORTE pin index: 26 */
#define BOARD_LED_GREEN_PORT PORTE                         /*!<@brief PORT device name: PORTE */
#define BOARD_LED_GREEN_PIN 26U                            /*!<@brief PORTE pin index: 26 */
#define BOARD_LED_GREEN_CHANNEL 26                         /*!<@brief GPIOE GPIO channel: 26 */
#define BOARD_LED_GREEN_PIN_NAME PTE26                     /*!<@brief Pin name */
#define BOARD_LED_GREEN_LABEL "J2[1]/D12[4]/LEDRGB_GREEN"  /*!<@brief Label */
#define BOARD_LED_GREEN_NAME "LED_GREEN"                   /*!<@brief Identifier name */
#define BOARD_LED_GREEN_DIRECTION kPIN_MUX_DirectionOutput /*!<@brief Direction */
                                                           /* @} */

/*!
 * @brief Configures pin routing and optionally pin electrical features.
 *
 */
void BOARD_InitLEDs(void);

#define SOPT5_UART0TXSRC_UART_TX 0x00u /*!<@brief UART 0 transmit data source select: UART0_TX pin */

/*! @name PORTB17 (number 63), U10[1]/UART0_TX
  @{ */
#define BOARD_DEBUG_UART_TX_PERIPHERAL UART0                   /*!<@brief Device name: UART0 */
#define BOARD_DEBUG_UART_TX_SIGNAL TX                          /*!<@brief UART0 signal: TX */
#define BOARD_DEBUG_UART_TX_PORT PORTB                         /*!<@brief PORT device name: PORTB */
#define BOARD_DEBUG_UART_TX_PIN 17U                            /*!<@brief PORTB pin index: 17 */
#define BOARD_DEBUG_UART_TX_PIN_NAME UART0_TX                  /*!<@brief Pin name */
#define BOARD_DEBUG_UART_TX_LABEL "U10[1]/UART0_TX"            /*!<@brief Label */
#define BOARD_DEBUG_UART_TX_NAME "DEBUG_UART_TX"               /*!<@brief Identifier name */
#define BOARD_DEBUG_UART_TX_DIRECTION kPIN_MUX_DirectionOutput /*!<@brief Direction */
                                                               /* @} */

/*! @name PORTB16 (number 62), U7[4]/UART0_RX
  @{ */
#define BOARD_DEBUG_UART_RX_PERIPHERAL UART0       /*!<@brief Device name: UART0 */
#define BOARD_DEBUG_UART_RX_SIGNAL RX              /*!<@brief UART0 signal: RX */
#define BOARD_DEBUG_UART_RX_PORT PORTB             /*!<@brief PORT device name: PORTB */
#define BOARD_DEBUG_UART_RX_PIN 16U                /*!<@brief PORTB pin index: 16 */
#define BOARD_DEBUG_UART_RX_PIN_NAME UART0_RX      /*!<@brief Pin name */
#define BOARD_DEBUG_UART_RX_LABEL "U7[4]/UART0_RX" /*!<@brief Label */
#define BOARD_DEBUG_UART_RX_NAME "DEBUG_UART_RX"   /*!<@brief Identifier name */
                                                   /* @} */

/*!
 * @brief Configures pin routing and optionally pin electrical features.
 *
 */
void BOARD_InitDEBUG_UART(void);

/*! @name PORTA18 (number 50), U13[16]/RMII_RXCLK
  @{ */
#define BOARD_EXTAL0_PERIPHERAL OSC             /*!<@brief Device name: OSC */
#define BOARD_EXTAL0_SIGNAL EXTAL0              /*!<@brief OSC signal: EXTAL0 */
#define BOARD_EXTAL0_PORT PORTA                 /*!<@brief PORT device name: PORTA */
#define BOARD_EXTAL0_PIN 18U                    /*!<@brief PORTA pin index: 18 */
#define BOARD_EXTAL0_PIN_NAME EXTAL0            /*!<@brief Pin name */
#define BOARD_EXTAL0_LABEL "U13[16]/RMII_RXCLK" /*!<@brief Label */
#define BOARD_EXTAL0_NAME "EXTAL0"              /*!<@brief Identifier name */
                                                /* @} */

/*! @name EXTAL32 (number 29), Y3[2]/EXTAL32_RTC
  @{ */
#define BOARD_ETAL32K_PERIPHERAL RTC            /*!<@brief Device name: RTC */
#define BOARD_ETAL32K_SIGNAL EXTAL32            /*!<@brief RTC signal: EXTAL32 */
#define BOARD_ETAL32K_PIN_NAME EXTAL32          /*!<@brief Pin name */
#define BOARD_ETAL32K_LABEL "Y3[2]/EXTAL32_RTC" /*!<@brief Label */
#define BOARD_ETAL32K_NAME "ETAL32K"            /*!<@brief Identifier name */
                                                /* @} */

/*! @name XTAL32 (number 28), Y3[1]/XTAL32_RTC
  @{ */
#define BOARD_XTAL32K_PERIPHERAL RTC           /*!<@brief Device name: RTC */
#define BOARD_XTAL32K_SIGNAL XTAL32            /*!<@brief RTC signal: XTAL32 */
#define BOARD_XTAL32K_PIN_NAME XTAL32          /*!<@brief Pin name */
#define BOARD_XTAL32K_LABEL "Y3[1]/XTAL32_RTC" /*!<@brief Label */
#define BOARD_XTAL32K_NAME "XTAL32K"           /*!<@brief Identifier name */
                                               /* @} */

/*!
 * @brief Configures pin routing and optionally pin electrical features.
 *
 */
void BOARD_InitOSC(void);

/*! @name PORTE25 (number 32), J2[18]/U8[6]/I2C0_SDA
  @{ */
#define BOARD_ACCEL_SDA_PERIPHERAL I2C0               /*!<@brief Device name: I2C0 */
#define BOARD_ACCEL_SDA_SIGNAL SDA                    /*!<@brief I2C0 signal: SDA */
#define BOARD_ACCEL_SDA_PORT PORTE                    /*!<@brief PORT device name: PORTE */
#define BOARD_ACCEL_SDA_PIN 25U                       /*!<@brief PORTE pin index: 25 */
#define BOARD_ACCEL_SDA_PIN_NAME I2C0_SDA             /*!<@brief Pin name */
#define BOARD_ACCEL_SDA_LABEL "J2[18]/U8[6]/I2C0_SDA" /*!<@brief Label */
#define BOARD_ACCEL_SDA_NAME "ACCEL_SDA"              /*!<@brief Identifier name */
                                                      /* @} */

/*! @name PORTE24 (number 31), J2[20]/U8[4]/I2C0_SCL
  @{ */
#define BOARD_ACCEL_SCL_PERIPHERAL I2C0               /*!<@brief Device name: I2C0 */
#define BOARD_ACCEL_SCL_SIGNAL SCL                    /*!<@brief I2C0 signal: SCL */
#define BOARD_ACCEL_SCL_PORT PORTE                    /*!<@brief PORT device name: PORTE */
#define BOARD_ACCEL_SCL_PIN 24U                       /*!<@brief PORTE pin index: 24 */
#define BOARD_ACCEL_SCL_PIN_NAME I2C0_SCL             /*!<@brief Pin name */
#define BOARD_ACCEL_SCL_LABEL "J2[20]/U8[4]/I2C0_SCL" /*!<@brief Label */
#define BOARD_ACCEL_SCL_NAME "ACCEL_SCL"              /*!<@brief Identifier name */
                                                      /* @} */

/*! @name PORTC6 (number 78), U8[11]/SW2
  @{ */
#define BOARD_ACCEL_INT1_PERIPHERAL GPIOC                  /*!<@brief Device name: GPIOC */
#define BOARD_ACCEL_INT1_SIGNAL GPIO                       /*!<@brief GPIOC signal: GPIO */
#define BOARD_ACCEL_INT1_GPIO GPIOC                        /*!<@brief GPIO device name: GPIOC */
#define BOARD_ACCEL_INT1_GPIO_PIN 6U                       /*!<@brief PORTC pin index: 6 */
#define BOARD_ACCEL_INT1_PORT PORTC                        /*!<@brief PORT device name: PORTC */
#define BOARD_ACCEL_INT1_PIN 6U                            /*!<@brief PORTC pin index: 6 */
#define BOARD_ACCEL_INT1_CHANNEL 6                         /*!<@brief GPIOC GPIO channel: 6 */
#define BOARD_ACCEL_INT1_PIN_NAME PTC6                     /*!<@brief Pin name */
#define BOARD_ACCEL_INT1_LABEL "U8[11]/SW2"                /*!<@brief Label */
#define BOARD_ACCEL_INT1_NAME "ACCEL_INT1"                 /*!<@brief Identifier name */
#define BOARD_ACCEL_INT1_DIRECTION kPIN_MUX_DirectionInput /*!<@brief Direction */
                                                           /* @} */

/*! @name PORTC13 (number 85), U8[9]
  @{ */
#define BOARD_ACCEL_INT2_PERIPHERAL GPIOC                  /*!<@brief Device name: GPIOC */
#define BOARD_ACCEL_INT2_SIGNAL GPIO                       /*!<@brief GPIOC signal: GPIO */
#define BOARD_ACCEL_INT2_GPIO GPIOC                        /*!<@brief GPIO device name: GPIOC */
#define BOARD_ACCEL_INT2_GPIO_PIN 13U                      /*!<@brief PORTC pin index: 13 */
#define BOARD_ACCEL_INT2_PORT PORTC                        /*!<@brief PORT device name: PORTC */
#define BOARD_ACCEL_INT2_PIN 13U                           /*!<@brief PORTC pin index: 13 */
#define BOARD_ACCEL_INT2_CHANNEL 13                        /*!<@brief GPIOC GPIO channel: 13 */
#define BOARD_ACCEL_INT2_PIN_NAME PTC13                    /*!<@brief Pin name */
#define BOARD_ACCEL_INT2_LABEL "U8[9]"                     /*!<@brief Label */
#define BOARD_ACCEL_INT2_NAME "ACCEL_INT2"                 /*!<@brief Identifier name */
#define BOARD_ACCEL_INT2_DIRECTION kPIN_MUX_DirectionInput /*!<@brief Direction */
                                                           /* @} */

/*!
 * @brief Configures pin routing and optionally pin electrical features.
 *
 */
void BOARD_InitACCEL(void);

#define SOPT2_RMIISRC_EXTAL 0x00u /*!<@brief RMII clock source select: EXTAL clock */

/*! @name PORTB1 (number 54), U13[11]/RMII0_MDC
  @{ */
#define BOARD_RMII0_MDC_PERIPHERAL ENET           /*!<@brief Device name: ENET */
#define BOARD_RMII0_MDC_SIGNAL RMII_MDC           /*!<@brief ENET signal: RMII_MDC */
#define BOARD_RMII0_MDC_PORT PORTB                /*!<@brief PORT device name: PORTB */
#define BOARD_RMII0_MDC_PIN 1U                    /*!<@brief PORTB pin index: 1 */
#define BOARD_RMII0_MDC_PIN_NAME RMII0_MDC        /*!<@brief Pin name */
#define BOARD_RMII0_MDC_LABEL "U13[11]/RMII0_MDC" /*!<@brief Label */
#define BOARD_RMII0_MDC_NAME "RMII0_MDC"          /*!<@brief Identifier name */
                                                  /* @} */

/*! @name PORTB0 (number 53), U13[10]/RMII0_MDIO
  @{ */
#define BOARD_RMII0_MDIO_PERIPHERAL ENET            /*!<@brief Device name: ENET */
#define BOARD_RMII0_MDIO_SIGNAL RMII_MDIO           /*!<@brief ENET signal: RMII_MDIO */
#define BOARD_RMII0_MDIO_PORT PORTB                 /*!<@brief PORT device name: PORTB */
#define BOARD_RMII0_MDIO_PIN 0U                     /*!<@brief PORTB pin index: 0 */
#define BOARD_RMII0_MDIO_PIN_NAME RMII0_MDIO        /*!<@brief Pin name */
#define BOARD_RMII0_MDIO_LABEL "U13[10]/RMII0_MDIO" /*!<@brief Label */
#define BOARD_RMII0_MDIO_NAME "RMII0_MDIO"          /*!<@brief Identifier name */
                                                    /* @} */

/*! @name PORTA13 (number 43), U13[13]/RMII0_RXD_0
  @{ */
#define BOARD_RMII0_RXD0_PERIPHERAL ENET             /*!<@brief Device name: ENET */
#define BOARD_RMII0_RXD0_SIGNAL RMII_RXD0            /*!<@brief ENET signal: RMII_RXD0 */
#define BOARD_RMII0_RXD0_PORT PORTA                  /*!<@brief PORT device name: PORTA */
#define BOARD_RMII0_RXD0_PIN 13U                     /*!<@brief PORTA pin index: 13 */
#define BOARD_RMII0_RXD0_PIN_NAME RMII0_RXD0         /*!<@brief Pin name */
#define BOARD_RMII0_RXD0_LABEL "U13[13]/RMII0_RXD_0" /*!<@brief Label */
#define BOARD_RMII0_RXD0_NAME "RMII0_RXD0"           /*!<@brief Identifier name */
                                                     /* @} */

/*! @name PORTA12 (number 42), U13[12]/RMII0_RXD_1
  @{ */
#define BOARD_RMII0_RXD1_PERIPHERAL ENET             /*!<@brief Device name: ENET */
#define BOARD_RMII0_RXD1_SIGNAL RMII_RXD1            /*!<@brief ENET signal: RMII_RXD1 */
#define BOARD_RMII0_RXD1_PORT PORTA                  /*!<@brief PORT device name: PORTA */
#define BOARD_RMII0_RXD1_PIN 12U                     /*!<@brief PORTA pin index: 12 */
#define BOARD_RMII0_RXD1_PIN_NAME RMII0_RXD1         /*!<@brief Pin name */
#define BOARD_RMII0_RXD1_LABEL "U13[12]/RMII0_RXD_1" /*!<@brief Label */
#define BOARD_RMII0_RXD1_NAME "RMII0_RXD1"           /*!<@brief Identifier name */
                                                     /* @} */

/*! @name PORTA5 (number 39), U13[17]/RMII0_RXER
  @{ */
#define BOARD_RMII0_RXER_PERIPHERAL ENET            /*!<@brief Device name: ENET */
#define BOARD_RMII0_RXER_SIGNAL RMII_RXER           /*!<@brief ENET signal: RMII_RXER */
#define BOARD_RMII0_RXER_PORT PORTA                 /*!<@brief PORT device name: PORTA */
#define BOARD_RMII0_RXER_PIN 5U                     /*!<@brief PORTA pin index: 5 */
#define BOARD_RMII0_RXER_PIN_NAME RMII0_RXER        /*!<@brief Pin name */
#define BOARD_RMII0_RXER_LABEL "U13[17]/RMII0_RXER" /*!<@brief Label */
#define BOARD_RMII0_RXER_NAME "RMII0_RXER"          /*!<@brief Identifier name */
                                                    /* @} */

/*! @name PORTA16 (number 46), U13[20]/RMII0_TXD0
  @{ */
#define BOARD_RMII0_TXD0_PERIPHERAL ENET            /*!<@brief Device name: ENET */
#define BOARD_RMII0_TXD0_SIGNAL RMII_TXD0           /*!<@brief ENET signal: RMII_TXD0 */
#define BOARD_RMII0_TXD0_PORT PORTA                 /*!<@brief PORT device name: PORTA */
#define BOARD_RMII0_TXD0_PIN 16U                    /*!<@brief PORTA pin index: 16 */
#define BOARD_RMII0_TXD0_PIN_NAME RMII0_TXD0        /*!<@brief Pin name */
#define BOARD_RMII0_TXD0_LABEL "U13[20]/RMII0_TXD0" /*!<@brief Label */
#define BOARD_RMII0_TXD0_NAME "RMII0_TXD0"          /*!<@brief Identifier name */
                                                    /* @} */

/*! @name PORTA17 (number 47), U13[21]/RMII0_TXD1
  @{ */
#define BOARD_RMII0_TXD1_PERIPHERAL ENET            /*!<@brief Device name: ENET */
#define BOARD_RMII0_TXD1_SIGNAL RMII_TXD1           /*!<@brief ENET signal: RMII_TXD1 */
#define BOARD_RMII0_TXD1_PORT PORTA                 /*!<@brief PORT device name: PORTA */
#define BOARD_RMII0_TXD1_PIN 17U                    /*!<@brief PORTA pin index: 17 */
#define BOARD_RMII0_TXD1_PIN_NAME RMII0_TXD1        /*!<@brief Pin name */
#define BOARD_RMII0_TXD1_LABEL "U13[21]/RMII0_TXD1" /*!<@brief Label */
#define BOARD_RMII0_TXD1_NAME "RMII0_TXD1"          /*!<@brief Identifier name */
                                                    /* @} */

/*! @name PORTA15 (number 45), U13[19]/RMII0_TXEN
  @{ */
#define BOARD_RMII0_TXEN_PERIPHERAL ENET            /*!<@brief Device name: ENET */
#define BOARD_RMII0_TXEN_SIGNAL RMII_TXEN           /*!<@brief ENET signal: RMII_TXEN */
#define BOARD_RMII0_TXEN_PORT PORTA                 /*!<@brief PORT device name: PORTA */
#define BOARD_RMII0_TXEN_PIN 15U                    /*!<@brief PORTA pin index: 15 */
#define BOARD_RMII0_TXEN_PIN_NAME RMII0_TXEN        /*!<@brief Pin name */
#define BOARD_RMII0_TXEN_LABEL "U13[19]/RMII0_TXEN" /*!<@brief Label */
#define BOARD_RMII0_TXEN_NAME "RMII0_TXEN"          /*!<@brief Identifier name */
                                                    /* @} */

/*! @name PORTA14 (number 44), U13[15]/RMII0_CRS_DV
  @{ */
#define BOARD_RMII0_CRS_DV_PERIPHERAL ENET              /*!<@brief Device name: ENET */
#define BOARD_RMII0_CRS_DV_SIGNAL RMII_CRS_DV           /*!<@brief ENET signal: RMII_CRS_DV */
#define BOARD_RMII0_CRS_DV_PORT PORTA                   /*!<@brief PORT device name: PORTA */
#define BOARD_RMII0_CRS_DV_PIN 14U                      /*!<@brief PORTA pin index: 14 */
#define BOARD_RMII0_CRS_DV_PIN_NAME RMII0_CRS_DV        /*!<@brief Pin name */
#define BOARD_RMII0_CRS_DV_LABEL "U13[15]/RMII0_CRS_DV" /*!<@brief Label */
#define BOARD_RMII0_CRS_DV_NAME "RMII0_CRS_DV"          /*!<@brief Identifier name */
                                                        /* @} */

/*! @name PORTA18 (number 50), U13[16]/RMII_RXCLK
  @{ */
#define BOARD_RMII_RXCLK_PERIPHERAL ENET            /*!<@brief Device name: ENET */
#define BOARD_RMII_RXCLK_SIGNAL RMII_CLKIN          /*!<@brief ENET signal: RMII_CLKIN */
#define BOARD_RMII_RXCLK_PORT PORTA                 /*!<@brief PORT device name: PORTA */
#define BOARD_RMII_RXCLK_PIN 18U                    /*!<@brief PORTA pin index: 18 */
#define BOARD_RMII_RXCLK_PIN_NAME EXTAL0            /*!<@brief Pin name */
#define BOARD_RMII_RXCLK_LABEL "U13[16]/RMII_RXCLK" /*!<@brief Label */
#define BOARD_RMII_RXCLK_NAME "RMII_RXCLK"          /*!<@brief Identifier name */
                                                    /* @} */

/*!
 * @brief Configures pin routing and optionally pin electrical features.
 *
 */
void BOARD_InitENET(void);

/*! @name PORTE0 (number 1), J15[P8]/SDHC0_D1
  @{ */
#define BOARD_SDHC0_D1_PERIPHERAL SDHC          /*!<@brief Device name: SDHC */
#define BOARD_SDHC0_D1_SIGNAL DATA              /*!<@brief SDHC signal: DATA */
#define BOARD_SDHC0_D1_PORT PORTE               /*!<@brief PORT device name: PORTE */
#define BOARD_SDHC0_D1_PIN 0U                   /*!<@brief PORTE pin index: 0 */
#define BOARD_SDHC0_D1_CHANNEL 1                /*!<@brief SDHC DATA channel: 1 */
#define BOARD_SDHC0_D1_PIN_NAME SDHC0_D1        /*!<@brief Pin name */
#define BOARD_SDHC0_D1_LABEL "J15[P8]/SDHC0_D1" /*!<@brief Label */
#define BOARD_SDHC0_D1_NAME "SDHC0_D1"          /*!<@brief Identifier name */
                                                /* @} */

/*! @name PORTE1 (number 2), J15[P7]/SDHC0_D0
  @{ */
#define BOARD_SDHC0_D0_PERIPHERAL SDHC          /*!<@brief Device name: SDHC */
#define BOARD_SDHC0_D0_SIGNAL DATA              /*!<@brief SDHC signal: DATA */
#define BOARD_SDHC0_D0_PORT PORTE               /*!<@brief PORT device name: PORTE */
#define BOARD_SDHC0_D0_PIN 1U                   /*!<@brief PORTE pin index: 1 */
#define BOARD_SDHC0_D0_CHANNEL 0                /*!<@brief SDHC DATA channel: 0 */
#define BOARD_SDHC0_D0_PIN_NAME SDHC0_D0        /*!<@brief Pin name */
#define BOARD_SDHC0_D0_LABEL "J15[P7]/SDHC0_D0" /*!<@brief Label */
#define BOARD_SDHC0_D0_NAME "SDHC0_D0"          /*!<@brief Identifier name */
                                                /* @} */

/*! @name PORTE2 (number 3), J15[P5]/SDHC0_DCLK
  @{ */
#define BOARD_SDHC0_DCLK_PERIPHERAL SDHC            /*!<@brief Device name: SDHC */
#define BOARD_SDHC0_DCLK_SIGNAL DCLK                /*!<@brief SDHC signal: DCLK */
#define BOARD_SDHC0_DCLK_PORT PORTE                 /*!<@brief PORT device name: PORTE */
#define BOARD_SDHC0_DCLK_PIN 2U                     /*!<@brief PORTE pin index: 2 */
#define BOARD_SDHC0_DCLK_PIN_NAME SDHC0_DCLK        /*!<@brief Pin name */
#define BOARD_SDHC0_DCLK_LABEL "J15[P5]/SDHC0_DCLK" /*!<@brief Label */
#define BOARD_SDHC0_DCLK_NAME "SDHC0_DCLK"          /*!<@brief Identifier name */
                                                    /* @} */

/*! @name PORTE3 (number 4), J15[P3]/SDHC0_CMD
  @{ */
#define BOARD_SDHC0_CMD_PERIPHERAL SDHC           /*!<@brief Device name: SDHC */
#define BOARD_SDHC0_CMD_SIGNAL CMD                /*!<@brief SDHC signal: CMD */
#define BOARD_SDHC0_CMD_PORT PORTE                /*!<@brief PORT device name: PORTE */
#define BOARD_SDHC0_CMD_PIN 3U                    /*!<@brief PORTE pin index: 3 */
#define BOARD_SDHC0_CMD_PIN_NAME SDHC0_CMD        /*!<@brief Pin name */
#define BOARD_SDHC0_CMD_LABEL "J15[P3]/SDHC0_CMD" /*!<@brief Label */
#define BOARD_SDHC0_CMD_NAME "SDHC0_CMD"          /*!<@brief Identifier name */
                                                  /* @} */

/*! @name PORTE4 (number 5), J15[P2]/SDHC0_D3
  @{ */
#define BOARD_SDHC0_D3_PERIPHERAL SDHC          /*!<@brief Device name: SDHC */
#define BOARD_SDHC0_D3_SIGNAL DATA              /*!<@brief SDHC signal: DATA */
#define BOARD_SDHC0_D3_PORT PORTE               /*!<@brief PORT device name: PORTE */
#define BOARD_SDHC0_D3_PIN 4U                   /*!<@brief PORTE pin index: 4 */
#define BOARD_SDHC0_D3_CHANNEL 3                /*!<@brief SDHC DATA channel: 3 */
#define BOARD_SDHC0_D3_PIN_NAME SDHC0_D3        /*!<@brief Pin name */
#define BOARD_SDHC0_D3_LABEL "J15[P2]/SDHC0_D3" /*!<@brief Label */
#define BOARD_SDHC0_D3_NAME "SDHC0_D3"          /*!<@brief Identifier name */
                                                /* @} */

/*! @name PORTE5 (number 6), J15[P1]/SDHC0_D2
  @{ */
#define BOARD_SDHC0_D2_PERIPHERAL SDHC          /*!<@brief Device name: SDHC */
#define BOARD_SDHC0_D2_SIGNAL DATA              /*!<@brief SDHC signal: DATA */
#define BOARD_SDHC0_D2_PORT PORTE               /*!<@brief PORT device name: PORTE */
#define BOARD_SDHC0_D2_PIN 5U                   /*!<@brief PORTE pin index: 5 */
#define BOARD_SDHC0_D2_CHANNEL 2                /*!<@brief SDHC DATA channel: 2 */
#define BOARD_SDHC0_D2_PIN_NAME SDHC0_D2        /*!<@brief Pin name */
#define BOARD_SDHC0_D2_LABEL "J15[P1]/SDHC0_D2" /*!<@brief Label */
#define BOARD_SDHC0_D2_NAME "SDHC0_D2"          /*!<@brief Identifier name */
                                                /* @} */

/*! @name PORTE6 (number 7), J15[G1]/SD_CARD_DETECT
  @{ */
#define BOARD_SDHC_CD_PERIPHERAL GPIOE                  /*!<@brief Device name: GPIOE */
#define BOARD_SDHC_CD_SIGNAL GPIO                       /*!<@brief GPIOE signal: GPIO */
#define BOARD_SDHC_CD_GPIO GPIOE                        /*!<@brief GPIO device name: GPIOE */
#define BOARD_SDHC_CD_GPIO_PIN 6U                       /*!<@brief PORTE pin index: 6 */
#define BOARD_SDHC_CD_PORT PORTE                        /*!<@brief PORT device name: PORTE */
#define BOARD_SDHC_CD_PIN 6U                            /*!<@brief PORTE pin index: 6 */
#define BOARD_SDHC_CD_CHANNEL 6                         /*!<@brief GPIOE GPIO channel: 6 */
#define BOARD_SDHC_CD_PIN_NAME PTE6                     /*!<@brief Pin name */
#define BOARD_SDHC_CD_LABEL "J15[G1]/SD_CARD_DETECT"    /*!<@brief Label */
#define BOARD_SDHC_CD_NAME "SDHC_CD"                    /*!<@brief Identifier name */
#define BOARD_SDHC_CD_DIRECTION kPIN_MUX_DirectionInput /*!<@brief Direction */
                                                        /* @} */

/*!
 * @brief Configures pin routing and optionally pin electrical features.
 *
 */
void BOARD_InitSDHC(void);

/*! @name USB0_DP (number 10), J22[3]/K64_MICRO_USB_DP
  @{ */
#define BOARD_USB_DP_PERIPHERAL USB0                 /*!<@brief Device name: USB0 */
#define BOARD_USB_DP_SIGNAL DP                       /*!<@brief USB0 signal: DP */
#define BOARD_USB_DP_PIN_NAME USB0_DP                /*!<@brief Pin name */
#define BOARD_USB_DP_LABEL "J22[3]/K64_MICRO_USB_DP" /*!<@brief Label */
#define BOARD_USB_DP_NAME "USB_DP"                   /*!<@brief Identifier name */
                                                     /* @} */

/*! @name USB0_DM (number 11), J22[2]/K64_MICRO_USB_DN
  @{ */
#define BOARD_USB_DM_PERIPHERAL USB0                 /*!<@brief Device name: USB0 */
#define BOARD_USB_DM_SIGNAL DM                       /*!<@brief USB0 signal: DM */
#define BOARD_USB_DM_PIN_NAME USB0_DM                /*!<@brief Pin name */
#define BOARD_USB_DM_LABEL "J22[2]/K64_MICRO_USB_DN" /*!<@brief Label */
#define BOARD_USB_DM_NAME "USB_DM"                   /*!<@brief Identifier name */
                                                     /* @} */

/*!
 * @brief Configures pin routing and optionally pin electrical features.
 *
 */
void BOARD_InitUSB(void);

#if defined(__cplusplus)
}
#endif

/*!
 * @}
 */
#endif /* _PIN_MUX_H_ */

/***********************************************************************************************************************
 * EOF
 **********************************************************************************************************************/
